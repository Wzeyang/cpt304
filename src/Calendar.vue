    ...
    <table cellpadding="5">
      ...
      <tr v-for="(day,k1) in days" style="{'animation-delay',(k1*30)+'ms'}">
        <td v-for="(child,k2) in day" :class="{'selected':child.selected,'disabled':child.disabled}"
          ...
          <span :class="{'red':weekend.indexOf(k2)!=-1||((child.isLunarFestival||child.isGregorianFestival ||child.isCustomDayLight ) && lunar)}">{{child.day}}</span>
  ...
  export default {
    ...
    methods: {
      ...
      getLunarInfo(y, m, d) {
        ...
        let isCustomDayLight = false
        if (this.festival.lunar[lunarInfo.lMonth + "-" + lunarInfo.lDay] != undefined) {
          lunarValue = this.festival.lunar[lunarInfo.lMonth + "-" + lunarInfo.lDay]
          isLunarFestival = true//此处为农历节日是否高亮
        } else if (this.festival.gregorian[m + "-" + d] != undefined) {
          lunarValue = this.festival.gregorian[m + "-" + d]
          isGregorianFestival = true //此处为阳历节日是否高亮
        }else if ( this.customDate[y + "-" + m + "-" + d] != undefined) {
          lunarValue = this.customDate[y + "-" + m + "-" + d]
          isCustomDayLight = true //自定义日期高亮
        }
        return {
          lunar: lunarValue,
